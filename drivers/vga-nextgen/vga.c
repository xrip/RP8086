...
// scanline_buffers: [0]=blank, [1]=vsync (CSYNC logic), [2]=odd image, [3]=even image
...
// --- Display geometry / timing ---
static int total_scanlines = 525;
static int visible_scanlines = 480;
static int vsync_start_line = 490;
static int vsync_end_line = 491;
static int picture_hshift_pixels = 0;
...
// --- Framebuffer / palette ---
static uint8_t *framebuffer_ptr = nullptr;

static uint16_t palette[256] __attribute__((aligned(4)));

// --- CSYNC MOD: Mask changed from 0xC0 to 0x40 ---
// Bit 6 is CSYNC (Active Low). Idle state during video is High (1).
...
void __time_critical_func() vga_scanline_dma() {
    ...
    static uint32_t current_scanline = 0;
    ...
    if (unlikely(current_scanline >= (uint32_t)visible_scanlines)) {
        ...
        // idx 0: Standard Blanking (CSYNC High, HSYNC pulses Low)
        // idx 1: VSYNC Active (CSYNC Low, Serration pulses High)
...
void graphics_set_palette(const uint8_t index, const uint32_t color) {
    ...
    // CSYNC MOD: Ensure bit 6 is High (Idle Sync) for active colors
    palette[index] = (rgb << 8 | rgb) & 0x3f3f | 0x4040;
}
...
void graphics_init() {
    ...
    pio_sm_set_consecutive_pindirs(PIO_VGA, sm, VGA_BASE_PIN, 7, true);
    ...
    sm_config_set_out_pins(&cfg, VGA_BASE_PIN, 7);
    ...
    constexpr uint8_t tmpl_active_idle = 0b01000000; // Active line, Sync High
    constexpr uint8_t tmpl_active_pulse = 0b00000000; // HSYNC pulse (Low)

    constexpr uint8_t tmpl_vsync_idle = 0b00000000; // VSYNC line active (Low)
    constexpr uint8_t tmpl_vsync_pulse = 0b01000000; // Serration pulse (High)

    constexpr int hsync_offset_pixels = 328 * 2;
    constexpr int hsync_pulse_width = 48 * 2;
    ...
    // 1. PREPARE STANDARD LINE (Buffer 0)
    auto base_ptr = (uint8_t *)scanline_buffers[0];
    // Set background to IDLE (High)
    memset(base_ptr, tmpl_active_idle, scanline_bytes);
    // Set HSYNC pulse to SYNC (Low) at start
    memset(base_ptr, tmpl_active_pulse, hsync_pulse_width);

    // 2. PREPARE VSYNC LINE (Buffer 1)
    base_ptr = (uint8_t *)scanline_buffers[1];
    // Set background to SYNC (Low)
    memset(base_ptr, tmpl_vsync_idle, scanline_bytes);
    // Set Serration pulse to IDLE (High) at start
    memset(base_ptr, tmpl_vsync_pulse, hsync_pulse_width);

    // 3. COPY TEMPLATES to Image Buffers (2 & 3)
    // They start as "Blank" standard lines, then filled with pixels in IRQ
