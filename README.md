...
|------|----------|---------------------|
| 0x40 | PIT Timer 0 (канал 0) | 0x0000 |
| 0x41 | PIT Timer 1 (канал 1) | 0x0000 |
| 0x61 | Speaker/PS/2 Control | 0x0000 |
| 0x21 | PIC Mask Register | 0x0000 |

**Общие порты:**
- Массив `ports[0xFFF]` для всех остальных портов (0x000-0xFFF)
...
| **'V'** | Дамп видеопамяти | Выводит 160x25 символов из видеопамяти MDA |
| **'P'** | Дамп портов | Выводит первые 400 байт массива `ports[]` |
| **'R'** | Сброс CPU | Выполняет последовательность сброса процессора |
...
- `i8086_read()`: 16-битное чтение памяти/I/O с BHE поддержкой
...
- `pic_init()`: Инициализирует INTR пин (GPIO26) и запускает Core1
- `core1_irq_generator()`: Работает на Core1, генерирует IRQ0 каждые ~54.925ms
- **INTA через PIO**: Полностью реализован в state machine (IRQ 3)
- **Синхронизация**: Использует volatile флаги для межъядерного взаимодействия
