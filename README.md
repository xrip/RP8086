...
| 25 | INTR | Выход | Interrupt request для i8086 (активный HIGH) |
| 26 | INTA | Вход | Interrupt acknowledge от i8086 (активный LOW) |
| 27 | RESET | Выход | Reset для i8086 (активный LOW) |
| 28 | READY | Выход | Wait state control для CPU (0=wait, 1=wait) |
...
├── pic.c/h             # Контроллер прерываний 8259A (INTR/INTA, Core1)
...
### pic.c
- `pic_init()` - Инициализация контроллера прерываний, запуск Core1
- `inta_gpio_callback()` - **Критичный обработчик** INTA протокола
  - Управление READY сигналом (wait state)
  - Остановка/перезапуск PIO SM
  - Выдача вектора прерывания 0x08
- `core1_irq_generator()` - Генератор IRQ0 на Core1 (~54.925ms, 18.2 Hz, IBM PC standard)
...
- ✅ Контроллер прерываний 8259A (INTR/INTA протокол, multicore)
